...
class DEICL(LMBase):
    def __init__(self, config):
        ...
        self.embedding_f = nn.Embedding(config.vocab_size, config.hidden_dim)
        self.embedding_phi = nn.Embedding(config.vocab_size, config.hidden_dim)
        
        self.x_s = nn.Parameter(torch.randn(1, 1, config.hidden_dim))
